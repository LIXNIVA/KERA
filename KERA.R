...
ui <- navbarPage(
  ...
  header = tagList(
    ...
    # ── Sub-header bar: subtitle + manual button ──────────────────────────────
  ...
  tabPanel("📁 Upload & Summary",
           div(class="page-wrap",
               ...
               # Logo — rendered server-side so it only shows if the file exists in www/
               uiOutput("logo_ui"),
               uiOutput("summary_ui")
           )
  ),
...
server <- function(input, output, session) {
  ...
  # ── Logo: only renders if KERA.png is present in www/ ─────────────────────
