...
ui <- navbarPage(
  ...
  tabPanel("📁 Upload & Summary",
           div(class = "page-wrap",
               ...
               # KERA Logo — served from logo/KERA.png via addResourcePath
...
# ── PMID detail modal (defined outside navbarPage so it is always available) ──
...
server <- function(input, output, session) {
  ...
  # Stores current modal data for download
  ...
  show_pmid_modal <- function(df_modal, title_label, accent) {
    ...
    showModal(modalDialog(
      ...
      footer = modalButton("Close"),
      # ── Override width to 92% of viewport ──
      tags$style(HTML(
        ".modal-dialog { max-width: 92vw !important; width: 92vw !important; }",
        ".modal-body   { padding: 20px 28px; }"
      )),
      # ── Header: title + download buttons ──
      div(
        style = paste0(
          "border-left:4px solid ", accent, ";padding:10px 16px;",
          "background:#f4f6f7;border-radius:6px;margin-bottom:16px;",
          "display:flex;align-items:center;justify-content:space-between;",
          "flex-wrap:wrap;gap:8px;"),
        span(style = "font-size:1.08em;font-weight:bold;color:#2c3e50;flex:1;",
             title_label),
        div(style = "display:flex;gap:8px;align-items:center;",
            tags$span(style = "font-size:12px;color:#7f8c8d;margin-right:2px;",
                      "Download:"),
  ...
  output$modal_dl_xlsx <- downloadHandler(
    filename = function()
      paste0(modal_title_rv(), "_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".xlsx"),
    content = function(file) {
      req(modal_data_rv())
      write_xlsx(modal_data_rv(), file)
    }
  )
  
  output$modal_dl_csv <- downloadHandler(
    filename = function()
      paste0(modal_title_rv(), "_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".csv"),
    content = function(file) {
      req(modal_data_rv())
      write.csv(modal_data_rv(), file, row.names = FALSE)
    }
  )
