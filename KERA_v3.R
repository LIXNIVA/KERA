...
evaluate_abstract <- function(abstract_text, api_key) {
  ...
  prompt <- paste0(
    ...
    "- dose_response: integer 0-100\n- temporal: integer 0-100\n- incidence: integer 0-100\n",
    "- note: one sentence max 25 words summarizing the key reasoning\n\n",
    "No explanation, no markdown, no code fences. Example:\n",
    "{\"dose_response\":75,\"temporal\":60,\"incidence\":30,\"note\":\"Wide dose range with significant response; short exposure window; lab study only.\"}\n\n",
    "Abstract:\n", abstract_text
  )
  payload <- list(model="claude-haiku-4-5-20251001", max_tokens=200,
  ...
  tryCatch({
    ...
    if (is.null(raw_text)) return(list(dose_response=NA, temporal=NA, incidence=NA, note=NA_character_))
    clean      <- trimws(gsub("```[a-zA-Z]*|```","",raw_text))
    json_match <- regmatches(clean, regexpr("\\{.*\\}", clean, perl=TRUE))
    if (length(json_match)==0) return(list(dose_response=NA, temporal=NA, incidence=NA, note=NA_character_))
    parsed <- fromJSON(json_match)
    list(dose_response=as.integer(parsed$dose_response), temporal=as.integer(parsed$temporal),
         incidence=as.integer(parsed$incidence), note=as.character(parsed$note))
  }, error=function(e) list(dose_response=NA, temporal=NA, incidence=NA, note=NA_character_))
}
...
server <- function(input, output, session) {
  ...
  output$ke_selector_ui <- renderUI({
    ...
    div(class="summary-box",
        ...
        fluidRow(
          ...
          column(1, div(class="arrow-center","🔗")),
          column(4, div(class="summary-box ke-box-taxa",
                        h4("🧬 Taxa / Species"),
                        selectInput("taxa",NULL,choices=c("(All / No filter)"="",evs),multiple=TRUE,selectize=TRUE),
  ...
  overlap_result <- eventReactive(input$search, {
    ...
    list(n_upstream=length(pmids_up), n_downstream=length(pmids_down),
         n_taxa=if(use_taxa) length(pmids_taxa) else NA,
         n_overlap=length(overlap_pmids), taxa_used=use_taxa, taxa_sel=taxa_sel,
         upstream_sel=input$upstream, downstream_sel=input$downstream,
         has_abstract="abstract" %in% colnames(df),
         pmid_summary=base_summary, overlap_rows=overlap_rows)
  })
    ...
    tagList(
      div(class="summary-box overlap-box",
          ...
          div(style="background:#f8f9fa;border-radius:8px;padding:12px;margin-bottom:18px;font-size:.95em;",
              tags$b("Upstream KE: "), tags$span(style="color:#e67e22;",paste(res$upstream_sel,collapse=", ")), tags$br(),
              tags$b("Downstream KE: "), tags$span(style="color:#27ae60;",paste(res$downstream_sel,collapse=", ")), tags$br(),
              tags$b("Taxa/Species: "), tags$span(style="color:#2980b9;",
          ...
          fluidRow(
            column(3, div(style="text-align:center;padding:15px;background:#fef9e7;border-radius:8px;",
                          p(class="stat-number",style="color:#e67e22;",res$n_upstream), p(class="stat-label","PMIDs — Upstream"))),
            column(3, div(style="text-align:center;padding:15px;background:#eafaf1;border-radius:8px;",
                          p(class="stat-number",style="color:#27ae60;",res$n_downstream), p(class="stat-label","PMIDs — Downstream"))),
            column(3, div(style="text-align:center;padding:15px;background:#eaf2fb;border-radius:8px;",
                          p(class="stat-number",style="color:#2980b9;",if(res$taxa_used) res$n_taxa else "—"), p(class="stat-label","PMIDs — Taxa/Species"))),
            column(3, div(style="text-align:center;padding:15px;background:#f5eef8;border-radius:8px;",
                          p(class="stat-number",style="color:#8e44ad;",res$n_overlap), p(class="stat-label","PMIDs — ALL Overlap")))
          ),
          tags$br(),
          if (res$n_overlap==0) {
            div(style="text-align:center;color:#e74c3c;padding:20px;", h4("⚠️ No overlapping PMIDs found."))
          } else {
            tagList(
              div(style="text-align:center;", tags$span(class="result-badge", paste0("✅ ",res$n_overlap," PMID(s) match all criteria"))),
              div(style="text-align:right;margin-bottom:10px;",
                  downloadButton("download_results","⬇️ Download Results (.xlsx)",
                                 style="background:#27ae60;border-color:#1e8449;color:white;")),
              h4(class="section-header", paste0("📋 Overlapping PMID Summary (",res$n_overlap," PMIDs)")),
  ...
  output$overlap_table <- renderDT({
    ...
    dt <- datatable(df_out,
                    options=list(pageLength=10, dom='ftp', scrollX=TRUE,
                                 columnDefs=if(res$has_abstract) list(list(targets=ncol(df_out)-1,
                                                                           render=JS("function(data,type,row){ if(type==='display'&&data&&data.length>120){",
                                                                                     "return '<span title=\"'+data+'\">'+data.substr(0,120)+'...</span>';} return data;}"))) else list()),
    ...
    if (res$has_abstract) dt <- dt %>% formatStyle("abstract",fontSize=".85em",color="#555")
    dt
  })
  ...
  count_event_pmids <- function(overlap_rows, event_list) {
    purrr::map_dfr(event_list, function(ev) {
      n <- overlap_rows %>%
        filter(trimws(event_1)==ev | trimws(event_2)==ev) %>% pull(PMID) %>% unique() %>% length()
      tibble(Event=ev, PMID_Count=n)
    })
  }
  
  df_upstream  <- reactive({ req(overlap_result(),input$us_top_n); res<-overlap_result()
  count_event_pmids(res$overlap_rows,res$upstream_sel) %>% arrange(desc(PMID_Count)) %>%
    head(input$us_top_n) %>% mutate(Event=factor(Event,levels=rev(Event))) })
  df_downstream<- reactive({ req(overlap_result(),input$ds_top_n); res<-overlap_result()
  count_event_pmids(res$overlap_rows,res$downstream_sel) %>% arrange(desc(PMID_Count)) %>%
    head(input$ds_top_n) %>% mutate(Event=factor(Event,levels=rev(Event))) })
  df_taxa      <- reactive({ req(overlap_result()); res<-overlap_result(); req(res$taxa_used)
  count_event_pmids(res$overlap_rows,res$taxa_sel) %>% arrange(desc(PMID_Count)) %>%
    mutate(Event=factor(Event,levels=rev(Event))) })
  df_year      <- reactive({ req(overlap_result()); overlap_result()$pmid_summary %>%
      mutate(Year=as.character(pubdate)) %>% count(Year,name="PMIDs") %>% arrange(Year) })
  heatmap_mat  <- reactive({
    req(overlap_result()); res<-overlap_result(); rows<-res$overlap_rows
    mat <- outer(res$upstream_sel, res$downstream_sel, FUN=Vectorize(function(u,d){
      rows %>% filter(PMID %in% (rows %>% filter(trimws(event_1)==u|trimws(event_2)==u) %>% pull(PMID))) %>%
        filter(trimws(event_1)==d|trimws(event_2)==d) %>% pull(PMID) %>% unique() %>% length()
    }))
    rownames(mat)<-res$upstream_sel; colnames(mat)<-res$downstream_sel; mat
  })
  
  output$viz_ui <- renderUI({
    req(overlap_result()); res<-overlap_result(); req(res$n_overlap>0)
    tagList(
      fluidRow(
        column(6, div(class="summary-box",style="border-top:4px solid #e67e22;",
                      h4(class="section-header","⬆️ Upstream Key Events — PMID Count"),
                      div(class="chart-ctrl", sliderInput("us_top_n","Show top N:",min=1,max=min(20,length(res$upstream_sel)+10),value=min(10,length(res$upstream_sel)+5),step=1)),
                      plotlyOutput("chart_upstream",height="380px"), dl_bar_ui("dl_us_tiff","dl_us_jpg"))),
        column(6, div(class="summary-box viz-box",
                      h4(class="section-header","⬇️ Downstream Key Events — PMID Count"),
                      div(class="chart-ctrl", sliderInput("ds_top_n","Show top N:",min=1,max=min(20,length(res$downstream_sel)+10),value=min(10,length(res$downstream_sel)+5),step=1)),
                      plotlyOutput("chart_downstream",height="380px"), dl_bar_ui("dl_ds_tiff","dl_ds_jpg")))
      ),
      fluidRow(
        column(6, div(class="summary-box",style="border-top:4px solid #2980b9;",
                      ...
                      else tagList(plotlyOutput("chart_taxa",height="380px"), dl_bar_ui("dl_taxa_tiff","dl_taxa_jpg")))),
        column(6, div(class="summary-box",style="border-top:4px solid #8e44ad;",
                      h4(class="section-header","📅 Overlapping PMIDs by Year"),
                      plotlyOutput("chart_year",height="380px"), dl_bar_ui("dl_year_tiff","dl_year_jpg")))
      ),
      fluidRow(
        column(12, div(class="summary-box",style="border-top:4px solid #16a085;",
                       ...
                       p("Each cell = overlapping PMIDs for each upstream–downstream pair.",style="color:#7f8c8d;font-size:.9em;"),
                       plotlyOutput("chart_heatmap",height="420px"), dl_bar_ui("dl_heat_tiff","dl_heat_jpg")))
      )
    )
  })
  
  output$chart_upstream   <- renderPlotly({ df<-df_upstream()
  plot_ly(df,x=~PMID_Count,y=~Event,type="bar",orientation="h",source="upstream_click",
    ...
    layout(xaxis=list(title="Number of PMIDs",zeroline=FALSE), yaxis=list(title="",automargin=TRUE),
           plot_bgcolor="#fafafa",paper_bgcolor="white",margin=list(l=10,r=20,t=10,b=40)) %>%
    config(displayModeBar=FALSE) %>% event_register("plotly_click") })
  
  output$chart_downstream <- renderPlotly({ df<-df_downstream()
  ...
  output$chart_taxa       <- renderPlotly({ df<-df_taxa()
  ...
  output$chart_year       <- renderPlotly({ df<-df_year()
  ...
  output$chart_heatmap    <- renderPlotly({ mat<-heatmap_mat()
  plot_ly(x=colnames(mat),y=rownames(mat),z=mat,type="heatmap",source="heatmap_click",
    ...
    layout(xaxis=list(title="Downstream Key Events",tickangle=-30,automargin=TRUE),
           ...
           plot_bgcolor="white",paper_bgcolor="white",margin=list(l=20,r=20,t=20,b=80)) %>%
    config(displayModeBar=FALSE) %>% event_register("plotly_click") })
  
  # High-res downloads
  make_dl_handler <- function(get_gg, base_name, fmt) {
    downloadHandler(
      filename=function() paste0(base_name,"_600dpi_",format(Sys.time(),"%Y%m%d_%H%M%S"),".",if(fmt=="tiff")"tiff" else "jpg"),
      content =function(file) {
        p <- get_gg(); n_events <- if(inherits(p$data,"data.frame")) nrow(p$data) else 10
        plot_h <- max(4, n_events*0.35+1.5)
        if (fmt=="tiff") ggsave(file,plot=p,device="tiff",dpi=600,width=10,height=plot_h,units="in",compression="lzw")
        else             ggsave(file,plot=p,device="jpeg",dpi=600,width=10,height=plot_h,units="in",quality=95)
      }
    )
  }
  ...
  show_pmid_modal <- function(df_modal, title_label, accent) {
    ...
    modal_data_rv(df_modal); modal_title_rv(gsub("[^A-Za-z0-9_-]","_",title_label))
    output$modal_pmid_table <- renderDT({
      dt <- datatable(df_modal, options=list(pageLength=10,dom="ftp",scrollX=TRUE),
                      rownames=FALSE, colnames=col_names, escape=FALSE) %>%
        formatStyle("Upstream_Events",color="#e67e22",fontWeight="bold") %>%
        formatStyle("Downstream_Events",color="#27ae60",fontWeight="bold") %>%
        formatStyle("Taxa_Species",color="#2980b9",fontWeight="bold")
    ...
    showModal(modalDialog(title=NULL, size="l", easyClose=TRUE, footer=modalButton("Close"),
                          ...
                          div(style=paste0("border-left:4px solid ",accent,";padding:10px 16px;background:#f4f6f7;border-radius:6px;",
                              ...
                              div(style="display:flex;gap:8px;align-items:center;",
                                  ...
                                  downloadButton("modal_dl_xlsx","XLSX",style="background:#27ae60;border-color:#1e8449;color:white;font-size:12px;padding:4px 14px;"),
                                  downloadButton("modal_dl_csv", "CSV", style="background:#2980b9;border-color:#1a5276;color:white;font-size:12px;padding:4px 14px;"))),
  ...
  pmids_for_event <- function(ev) {
    res <- overlap_result()
    pmids <- res$overlap_rows %>% filter(trimws(event_1)==ev|trimws(event_2)==ev) %>% pull(PMID) %>% unique()
  ...
  df<-isolate(overlap_result()$pmid_summary %>% mutate(yr_str=as.character(pubdate)) %>%
  ...
  df<-isolate({ res<-overlap_result(); rows<-res$overlap_rows
  ...
  output$kera_status_ui <- renderUI({
    cls <- switch(rv$status, idle="status-bar status-idle", running="status-bar status-running",
                  done="status-bar status-done", error="status-bar status-error")
  ...
  output$kera_results_header <- renderUI({
    ...
    div(style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;",
        ...
        div(style="display:flex;gap:8px;",
            ...
            if (nsel>0) div(style="font-family:'Courier New',monospace;font-size:11px;color:#2563eb;background:#eff6ff;border:1px solid #bfdbfe;border-radius:20px;padding:3px 11px;",
  ...
  output$kera_table_ui <- renderUI({
    if (is.null(rv$results))
      return(div(class="empty-state", div(class="empty-icon","◈"), div(class="empty-text","Awaiting evaluation")))
  ...
  observeEvent(input$run_btn, {
    ...
    tryCatch({
      ...
      # Case-insensitive column matching — accepts "abstract", "Abstract", "ABSTRACT" etc.
      col_match <- names(df)[tolower(names(df)) == tolower(col_input)]
      if (length(col_match) == 0) { rv$status<-"error"; rv$message<-paste0("Column '",col_input,"' not found. Available: ",paste(names(df),collapse=", ")); return() }
      col <- col_match[1]
      ...
      out_df[["Note"]]                          <- notes
      rv$results<-out_df; rv$status<-"done"; rv$message<-paste0("✓ Done — ",n," abstracts evaluated")
    ...
    datatable(df, escape=FALSE, rownames=FALSE,
              ...
              options=list(pageLength=10, scrollX=TRUE, dom="frtip",
                           columnDefs=list(
                             ...
                             list(width="220px", targets=which(names(df)=="Note")-1),
  ...
  output$pmid_btn <- downloadHandler(
    ...
    content =function(file) {
      ...
      writeLines(c("# KERA Selected PMIDs", paste0("# Generated: ",format(Sys.time(),"%Y-%m-%d %H:%M:%S")),
